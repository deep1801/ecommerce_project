...
const AdminProducts = () => {
  ...
  return (
    <div className="min-h-screen px-5 sm:px-6 py-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 font-display text-3xl font-extrabold text-ink-950 sm:text-4xl">
              Manage Products
            </h1>
        ...
        <div className="mb-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Total Products", value: products.length, accent: "text-ink-950" },
            { label: "Search Results", value: filteredProducts.length, accent: "text-ink-950" },
            { label: "Average Price", value: `₹${averagePrice.toLocaleString("en-IN")}`, accent: "text-ink-950" },
            { label: "Inventory Value", value: `₹${totalPrice.toLocaleString("en-IN")}`, accent: "text-emerald-600" },
          ].map((s) => (
            <div
              key={s.label}
        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredProducts.map((product) => (
              <div
                key={product._id}
                className="group overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-card"
              >
                ...
                <div className="p-5">
                  <h2 className="line-clamp-1 font-display font-bold text-ink-950">
                    {product.title}
                  </h2>
                  ...
                  <div className="mt-4 flex gap-2">
                    <button
                      ...
                      className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-ink-200 py-2.5 text-sm font-semibold text-ink-950 transition hover:bg-ink-950 hover:text-white"
                    >
                      <svg
                        className="h-4 w-4"
                        fill="none"
